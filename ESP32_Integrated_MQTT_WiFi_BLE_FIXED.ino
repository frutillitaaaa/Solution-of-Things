/*
 * ESP32 Integrado: WiFi + BLE + MQTT para "MyPaws"
 * VERSIÓN CORREGIDA - BLE se inicia correctamente
 * 
 * Este sketch combina:
...
void iniciarBLE() {
  ...
  Serial.println("📱 Busca 'PetFeeder BLE' en tu dispositivo");
  Serial.println("📤 Envía 'start_ap' para iniciar configuración WiFi");
}
...
bool intentarConexionWiFi() {
  ...
  if (ssid == "") {
    Serial.println("⚠️ No hay WiFi guardado.");
    return false;
  }
...
void setup() {
  ...
  // Si no hay WiFi, iniciar BLE
  if (!wifiSuccess) {
    Serial.println("🔵 Iniciando BLE para configuración WiFi...");
    iniciarBLE();
  }
